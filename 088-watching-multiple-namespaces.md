...
In [issue #5895](https://github.com/strimzi/strimzi-kafka-operator/issues/5895), or [issue #879](https://github.com/strimzi/strimzi-kafka-operator/issues/879) there was a discussion about how to declare a `KafkaUser`, `kafkaTopic` in other namespace than the one where the user-operator is deployed, this need is also right for `KafkaConnector`.
...
With Strimzi we have the capacity to create a Helm chart for our applications which automatically deploye and configure the customer KafkaUser, kafkaTopic and KafkaConnector, but the problem is than all customer must be deployed in the same namespace where `Kafka` resource is deployed and so in the same namespace of all customer. So with thousands of customers the read and management of this namespace become difficult or impossible to use while having thousands namespaces it's easier to manage. Of Kubernetes point of view this allows to separate `Kafka` components in dedicated namespace, configure quota and limit on namespace, secure namespace (customers and Kafka namespace) with network policies but particularly to secure and separate all client applications that use Kafka.
