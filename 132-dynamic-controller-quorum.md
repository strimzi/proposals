...
* If the node is a controller:
    ...
    * Checks if the current controller's ID is in the controllers list:
      * **Controller in list**:
        * If metadata disk change is detected, formats with `-N` to generate a new directory ID (handles disk failures or storage replacement scenarios).
        * Otherwise, formats with `-I` to preserve the directory ID specified in the controllers string. This handles initial bootstrap, rolling restarts, PVC replacements, and disaster recovery scenarios.
      * **Controller not in list**:
        * Formats with `-N` to generate a new random directory ID (new controller being added during scale-up).
