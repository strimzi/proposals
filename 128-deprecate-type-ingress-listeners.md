...
This proposal suggests the deprecation of the `type: ingress` listener API in the `Kafka` custom resources.
It does not suggest any deadline for removal of the code it depends on.
...
The API will be removed in the future in the `v2` API.
...
_Note: We are just moving to `v1` API, so `v2` API is likely several years away._
...
While we will mark the `type: ingress` listeners as deprecated, it will have no impact (other than the deprecation warnings) on the Kafka clusters using it.
...
They might not care about the bugs, CVEs, and outdated dependencies (for example in disconnected environments where the security risk is mitigated).
...
### Supporting other Ingress controller
