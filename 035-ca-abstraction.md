...
The client CA root certificate is added to the broker trust stores too, so that the brokers will trust certificates issued to end users. 
...
Removing the assumption that Strimzi always issues EE certificates using a CA certificate it has access to it would enable much more flexibility in CA handling.
...
(In the `openssl` implementation this might encode the CSR).
...
```
  NEW // a certificate is needed for the end entity, but doesn't exist
   |
   v
  REQUESTED // a certificate has been requested
   |
   v
  PENDING // a certificate has been issued, but the root CA is not trusted yet
   |
   v
