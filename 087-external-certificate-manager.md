...
Strimzi will annotate the Secret it manages with:
* `strimzi.io/user-cert-hash` annotation with the value being the hash of the certificate in the cert-manager Secret.
* `strimzi.io/clients-ca-cert-generation` annotation with the value matching the current value on the `<CLUSTER_NAME>-clients-ca-cert` Secret.

#### Tracking changes to user certificates

Cert-manager will be responsible for renewing all user certificates.
When a certificate is renewed cert-manager will update the related Secret.
During a reconciliation Strimzi will check the hash of the certificate stored in cert-manager Secrets to see if an update is needed.
Strimzi will copy over the new certificate into its own Secret and update the `strimzi.io/user-cert-hash` annotation to match the new certificate.

> ![Issuing new user certificate](./images/087-new-renew-user-cert.png)
>
> Fig 3: Proposed workflow when cert-manager issues a new Kafka User certificate
...
During the user operator reconciliation loop Strimzi will copy the new CA cert into any user certificate Secrets, incrementing the `strimzi.io/clients-ca-cert-generation`.
