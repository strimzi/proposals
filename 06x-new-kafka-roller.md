...
7. Reconfigure each node in `RECONFIGURE` group:
   ...
   - Wait for each node that got configurations updated until they have `SERVING` within the `postOperationalTimeoutMs`. 
...
TODO: we need to figure out how to work out the total number of controller without relying on the describe API.
