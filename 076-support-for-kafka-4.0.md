...
This proposal discusses how Strimzi will handle the release of Apache Kafka 4.0 and transition from Apache Kafka 3.x to 4.x.
...
When new minor Apache Kafka version is released, Strimzi will typically adopt the new minor version and drop support for the oldest support Kafka minor version.
Typically, this happens every other Strimzi release as Apache Kafka has slower release cycle compared to Strimzi.
...
Also, after they are released in Apache Kafka, additional time might be needed to adopt (use) them in Strimzi.
...
2) Given the expected timeline of 3-4 months between Kafka 3.9 and 4.0, we will do another minor Strimzi release 0.4y.
   ...
   It will also keep the warnings introduced in previous Strimzi version.
...
3) Once Strimzi 0.4y is released, we will proceed and remove ZooKeeper support from Strimzi.
   ...
   * Remove ZooKeeper related system tests
   ...
   Strimzi 0.4z will support the last two Kafka minor versions as usually - Kafka 3.9 and 4.0 - and will allow users to gradually upgrade as always.
...
4) The Strimzi releases after 0.4z will continue as usually with the 1-2 months release cycles and will continue to adopt the Kafka patch releases.
...
(While it is rare, it is possible that for example addressing some CVEs might require significant dependency updated forcing major changes to the code base)
...
Leaving aside the questions about the quality of the KRaft implementation and the missing features, the biggest risk are last-minute changes to the Apache Kafka release timelines and general plans.
...
But it to some extend affects also other parts of the project (such as the API, container images, examples, system tests etc.)
...
But assuming the interval between Kafka 3.9 and 4.0 is 3-4 months, it seems that keeping the 0.4y release can help us implemented some more features and possibly fix some more bug fixes before we drop ZooKeeper support.
Doing the 0.4y release would also minimize the risk of Apache Kafka introducing 3.10 release only later after the 3.9 release is done.
This should also simplify the extended support, since we will have to do it for a shorter time in a separate release branch and safe some effort (from the 1 year of support window of Kafka 3.9, 1-2 months will be gone before we release the 0.4y with extended support).
