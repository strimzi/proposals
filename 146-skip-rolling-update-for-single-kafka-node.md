...
- **Precedence vs `strimzi.io/manual-rolling-update`.** The two are opposites (force vs suppress a roll). If both apply to a node, **skip wins** (conservative — the point is to keep the operator off a flagged node), with a warning event + `.status` condition flagging the contradiction. Crucially this must be enforced at **two call sites**: manual rolling update runs in its own early phase (`KafkaReconciler.manualRollingUpdate()`, built from pod/`StrimziPodSet` annotations) *before* the main `rollingUpdate()`, so skipped nodes must be filtered out of **both** the early manual set and the main working set — enforcing it only in the main roll would still let the early phase restart a skipped node. Note the early set has two sources: per-pod annotations *and* a `StrimziPodSet`-level annotation that `podsForManualRollingUpdateDiscoveredThroughPodSetAnnotation` **expands to every node in the set**. The skip filter must be subtracted from the *expanded* set, not just the pod-annotation list — otherwise a podset-wide manual-rolling-update annotation would re-introduce the skipped node.
...
For a skipped (broker) node, the predicate gates **membership in the working set**: the node is excluded when the roller builds the list it iterates (the `controllerPods`/`brokerPods` ordering in `KafkaRoller.rollingRestart`, fed from `kafka.nodes()` via `KafkaReconciler.maybeRollKafka`). That exclusion removes it from the roll-candidate set and from the roller's *internal* readiness waits, so the node is never force-restarted: it cannot reach the force paths (`markRestartContextWithForceRestart()` after `POD_UNRESPONSIVE`, or `POD_FORCE_RESTART_ON_ERROR` in the `catch (ForceableProblem)` branch), nor the `FatalProblem`-mapped readiness waits inside the roller (the "does not need restart" branch of `restartIfNecessary`, and the post-restart `awaitReadiness`). The roller still evaluates and, where safe, rolls the *other* nodes (kept safe by the existing availability check; see "Availability math").

This handles the roller, but **not** the whole reconcile: `KafkaReconciler.reconcile()` calls `podsReady()` as a *separate* stage after `rollingUpdate()`, and that stage waits on the unfiltered `kafka.nodes()` set. Working-set exclusion alone therefore stops the destructive restart loop but still lets the reconcile time out on the skipped pod every cycle — so v1 must also exclude the node there (next section). **This is the v1 behavior.**
...
- Even with zero reasons, an in-scope not-ready node still drives the roller's force/await machinery. The first readiness wait (`await(isReady(pod), operationTimeoutMs, RuntimeException::new)`) is *caught* and routed into the not-ready handling, which is where `POD_UNRESPONSIVE` → force-restart originates; separately, the `FatalProblem`-mapped waits (the "does not need restart" branch of `restartIfNecessary` and the post-restart `awaitReadiness`) abort the whole roll for an in-scope node. A reason filter touches none of this — only removing the node from the working set does.
...
### Reconcile readiness stage and CR status

Excluding the node from `KafkaRoller` is not enough on its own. `KafkaReconciler.reconcile()` runs an explicit `podsReady()` stage *after* `rollingUpdate()`, and `podsReady()` waits on the full, unfiltered `kafka.nodes()` set (via `ReconcilerUtils.podsReady`, which joins a per-pod readiness future per node and fails the stage if any pod is not ready within `STRIMZI_OPERATION_TIMEOUT_MS`). So without a second change, a skipped-but-NotReady node still:

- makes `podsReady()` time out after the full operation timeout **every** reconcile (trading the restart loop for a reconcile-failure loop), and
- short-circuits the pipeline at that stage, so the post-readiness steps (`clusterId`, quotas, KRaft node unregistration, metadata version, `deletePersistentClaims`, status finalization) never run.

v1 therefore applies the same skip predicate to the readiness wait: **`podsReady()` (and the endpoint-readiness stages `serviceEndpointsReady` / `headlessServiceEndpointsReady`) operate on `kafka.nodes()` minus skipped nodes.** This mirrors the working-set exclusion so the skipped node never blocks the reconcile.

**CR status contract (explicit).** With the skipped node removed from the readiness stage, the reconcile completes and the `Kafka` CR's `Ready` condition reflects only the *non-skipped* nodes. The skip is surfaced as a distinct, non-fatal `NodeRollingUpdateSkipped` condition (see "Status reporting") and is **not** folded into `Ready` — so a healthy cluster with one intentionally-skipped node is `Ready=True` with an informational skip condition, not stuck `NotReady`. The genuine state of the skipped node (still `0/1` from kubelet probes) remains visible on the pod and via the skip condition and metrics; the operator simply stops gating cluster readiness on it.
...
The sharper cert risk is **CA replacement**, not just expiry. If the cluster CA is replaced (e.g. `strimzi.io/force-replace`) while a node is skipped, the rest of the cluster rolls onto the new CA's trust while the skipped node keeps its old key material and trust set. The skipped node is not rolled to pick up the new certs, so when it is *later* restarted by something outside the operator (kubelet, an eviction, a host reboot) it may fail to authenticate/rejoin the cluster — a failure that surfaces well after the skip was set. This is more dangerous than simple expiry because it can break a node that looked fine while skipped.

Handling: surface a `.status` condition when a skipped node nears its cert-renewal deadline (reusing the window the operator already tracks), when a CA renewal/replacement has advanced the cluster past a skipped node, or when it has a pending config/version change blocked by the skip; **do not auto-unskip** (that reintroduces the suppressed restart — the human decides, prompted by the condition); and document that a skip must not be held across a CA renewal/replacement and is meant to be short-lived, not a permanent exclusion.
...
- **`KafkaRoller` / `KafkaReconciler` unit tests.** A skipped NotReady broker (for any reason, not only `RECOVERY`) is not rolled and is excluded from the working set, so it never reaches the force paths or the roller's `FatalProblem` waits. Critically, a skipped NotReady broker is also excluded from the `podsReady()` stage so the **whole reconcile completes** and the `Kafka` CR is `Ready=True` (the core regression test — without the `podsReady()` exclusion the reconcile times out and short-circuits every cycle); the `NodeRollingUpdateSkipped` condition is present and is not folded into `Ready`. A skipped broker with a pending config/cert reason is not rolled. No global deadlock: a broker sharing no at-risk partition still rolls and the reconcile completes the safe subset, while a roll that would breach min-ISR is deferred via the *existing* availability check (confirming v1 needs no math change). Precedence: skip wins at **both** call sites — including a `StrimziPodSet`-level `manual-rolling-update` expanded to all nodes — with the contradiction warning. Parse-failure: invalid `nodeIds` ignored, valid ones applied, warning surfaced. Controller refusal: a controller-targeting skip (controller-only and combined) is refused with `SkipRollingUpdateRefused` and still managed.
...
Affected:
- `strimzi/strimzi-kafka-operator` — the cluster operator. In v1: `KafkaRoller`/`KafkaReconciler` working-set construction (excluding skipped broker nodes from both the main roll and the expanded early `manualRollingUpdate()` set); **`KafkaReconciler.podsReady()` and the endpoint-readiness stages (`serviceEndpointsReady` / `headlessServiceEndpointsReady`)**, which today wait on the unfiltered `kafka.nodes()` set and must also exclude skipped nodes (without this the reconcile times out on the skipped pod every cycle); the reconcile pipeline that reads the pod annotation and `KafkaNodePool.spec.skipRollingUpdate` field; the `KafkaNodePool` CRD (new `spec` field); `Kafka` / `KafkaNodePool` `.status` conditions (including controller `SkipRollingUpdateRefused`); new metrics; and the `SkipNodeRollingUpdate` feature gate. **Not** changed in v1: `KafkaAvailability` (the ISR-subtraction refinement is a follow-up), `KafkaQuorumCheck` / controller quorum math (controller skips are refused), and `KafkaReconciler.scaleDown()` (a skip does not govern membership — see "What a skip does not stop").
