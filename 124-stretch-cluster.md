...
To prevent unsuitable stretch cluster deployments that would result in poor performance and support burden, the operator performs automated network latency validation on initial deployment.

**Purpose:**  
Blocks deployments across geographic regions or high-latency networks before resources are created, preventing users from deploying stretch clusters in environments where Kafka cannot operate reliably.

**How it works:**
1. Deploy lightweight test pods (`busybox`) in both central and remote clusters
2. Expose remote test pods using the configured networking provider (NodePort/LoadBalancer/MCS)
3. Execute actual TCP connection tests from central pod to remote pods
4. Measure real network packet round-trip time (RTT) using `nc` (netcat)
5. Take multiple samples (5 per cluster) and use median for robustness
6. Validate measurements against configurable thresholds
7. Clean up all test resources (pods and networking resources)
8. Block deployment if latency exceeds limits; allow with warnings if elevated but acceptable

**Configuration:**
```bash
# Operator environment variables
STRIMZI_STRETCH_MAX_LATENCY_MS=10         # Block if >10ms (default)
STRIMZI_STRETCH_WARNING_LATENCY_MS=5      # Warn if >5ms (default)
```

**User Experience:**

If deployment is blocked due to high latency:
```yaml
status:
  conditions:
  - type: NetworkLatencyValidation
    status: "False"
    reason: NetworkLatencyTooHigh
    message: |
      Kafka cluster 'my-cluster' failed network latency validation:
      Cluster 'eu-cluster': 97ms median latency exceeds maximum 10ms 
      (min=95ms, median=97ms, avg=96ms, max=99ms)
      
      Stretch clusters require same datacenter deployment with network latency 
      below 10ms. cross region or high-latency deployments are not supported.
      
      Deployment blocked to prevent performance issues and instability.
      
      To fix this issue:
      1. Deploy all Kubernetes clusters in the same datacenter/region
      2. Ensure network infrastructure supports low-latency pod-to-pod communication
      3. Verify network path between clusters (check firewalls, routing, etc.)
      4. If using cloud providers, ensure all clusters are in the same region/zone
      
      Measured latencies: central <-> eu-cluster: min=95ms, median=97ms, avg=96ms, 
      max=99ms (endpoint: 10.0.0.1:31001) - EXCEEDS LIMIT;
  - type: Ready
    status: "False"
    reason: NetworkLatencyValidationFailed
    message: "Stretch cluster deployment blocked due to network latency validation 
             failure. See NetworkLatencyValidation condition for details."
```

No Kafka resources are created when validation fails, preventing wasted time and resources.

**Why this approach proposed:**  
- Tests actual pod-to-pod network latency (data plane), not Kubernetes API latency (control plane)
- Uses real TCP connections, measuring what Kafka will actually experience
- Runs only on initial deployment (no overhead on normal reconciliations)
- Provider agnostic (works with NodePort, LoadBalancer, MCS)
- Statistical rigor (multiple samples, median calculation)
- Self documenting (error messages explain requirements and fixes)
- Reduces support burden (prevents 80% of latency-related user errors)

**Implementation Considerations:**

Several approaches were evaluated for validating network latency in stretch clusters:

1. ICMP Ping / API server ping - Rejected: Tests control plane connectivity. 
API servers run on different networks than worker nodes where Kafka pods run.

2. Kubernetes API timing (Service creation/discovery time) - Rejected: Measures control plane latency (API/etcd operations, 100-500ms overhead), not data plane (pod-to-pod packets). 
Service can be created quickly while network has 95ms latency.

3. Static configuration (users declare latency) - Rejected: Unverifiable, error-prone. 
Users could declare "2ms" with actual 95ms latency.

4. Ongoing monitoring post-deployment - Rejected: By the time high latency is detected, Kafka already experiencing replication lag or instability. 
Fail fast is better.

5. Synthetic pod-to-pod test (chosen) - Selected: Tests actual data plane (TCP packets between pods). 
Validates provider configuration. 
Fails fast before Kafka resources created. Provider agnostic.

**Technical Implementation:**

The implementation integrates into the reconciliation flow:

- **Execution point**: After initial configuration validation, before any Kafka resource creation (in `reconcileStretchedKafka()` method)
- **Validation trigger**: Runs only when `kafka.status.observedGeneration` is null (initial deployment)
- **Async flow**: All operations return Vert.x `Future` objects, composed asynchronously to avoid blocking the event loop

**Measurement approach**:

1. **Deploy test pods** in each cluster using `NetworkLatencyTester`:
   - Remote cluster: `busybox` pod listening on port 8080 (`nc -l -p 8080 -k`)
   - Central cluster: `busybox` pod for executing connection tests
   - Minimal resources: 10m CPU, 32Mi memory per pod
   - Wait for pods to be ready (timeout: 60 seconds)

2. **Expose remote pods** using `provider.createNetworkingResources()`:
   - Creates Service (type depends on provider: NodePort/LoadBalancer/ClusterIP)
   - Waits for networking resources to be provisioned
   - For LoadBalancer: waits for external IP assignment

3. **Discover pod endpoints** using `provider.discoverPodEndpoint()`:
   ```java
   // Get the actual endpoint Kafka will use
   Future<String> endpoint = provider.discoverPodEndpoint(
       reconciliation, namespace, testPodName, clusterId, "test"
   );
   // Returns: "10.21.37.21:31001" (NodePort), "10.21.50.10:8080" (LB), 
   //          or "test.cluster-2.svc.clusterset.local:8080" (MCS)
   ```

4. **Execute network latency test** from central cluster to each remote cluster:
   - Command: `nc -zv -w 5 <host> <port>` (TCP connection test with 5s timeout)
   - Measures TCP handshake time (SYN/SYN-ACK/ACK round-trip)
   - Takes 5 samples per cluster with 100ms delay between samples
   - Calculates statistics: min, median, max, average
   - Uses **median** for threshold comparison (more robust than average to outliers)

5. **Validate measurements** against configured thresholds:
   - Compare median latency against `maxLatencyMs` (blocks if exceeded)
   - Compare median latency against `warningLatencyMs` (warns if exceeded)
   - Record all statistics (min/median/max/avg) for observability

6. **Clean up test resources** using `provider.deleteNetworkingResources()`:
   - Delete test pods in both central and remote clusters
   - Delete networking resources (Services, etc.)
   - Cleanup happens even on failure (best-effort)

**Result handling**:

- Returns `ValidationResult` object containing:
  - `passed`: boolean indicating success/failure
  - `measurements`: List of per-cluster latency stats (min/median/max/avg)
  - `errorMessage`: Detailed error with affected clusters and remediation steps
  - `warnings`: List of non-blocking warnings

**Key Design Decisions:**

- **One-time validation**: Runs only on initial deployment (`observedGeneration == null`).
Avoids reconciliation overhead on every reconciliation loop.
Network topology rarely changes after initial deployment.

- **Real pod-to-pod testing**: Uses actual TCP packets between pods to measure **data plane latency**, not Kubernetes control plane (API) latency.
Tests what Kafka will actually experience during replication.
Each test performs TCP handshake (SYN/SYN-ACK/ACK) which mirrors Kafka's connection establishment.

- **Provider-agnostic**: Works with any `StretchNetworkingProvider` implementation.
Tests use the provider's own endpoint discovery.
Ensures validation matches production configuration.
Provider creates the networking resources (Service type matches production).

- **Fail fast**: Validation happens **before** any Kafka resources are created.
Prevents wasted time waiting for Kafka pods that will never work.
Prevents orphaned resources in multiple clusters.
Clear error messages guide users to fix issues before retry.

- **Mandatory in stretch mode**: Validation must complete successfully before any Kafka resources are created.
Deployment is blocked if validation fails.
Prevents misconfigured deployments that will cause Kafka instability.
This ensures only suitable network environments are used for stretch clusters, reducing support burden and preventing production incidents.

- **Detailed observability**: All measurements recorded in status conditions.
Cluster-by-cluster breakdown with min/median/max/avg RTT. 
Users can see exactly which cluster pairs have issues. 
Measurements help users understand their network topology.

- **Timeout protection**: Entire validation has configurable timeout (default: 60 seconds).
Individual operations have sub-timeouts (pod ready: 60s, latency test: 30s).
If any test hangs (network unreachable, firewall blocking, etc.), validation fails gracefully.
Timeout errors include clear message about which cluster/operation timed out.

- **Statistical rigor**: Multiple samples (default: 5) account for network variance.
Uses **median** for threshold comparison (more robust than average).
Records min/max to show variance (helps diagnose network instability).
Small delay between samples (100ms) avoids burst effects.
